...
<div class="min-h-screen flex flex-col">
    ...
    <div class="flex flex-1">
        <!-- Sidebar -->
        <x-sidebar :project="(object)['id' => 1, 'name' => 'Project Dummy']" :boards="[
            ['id' => 1, 'name' => 'Board#1'],
            ['id' => 2, 'name' => 'Board#2']
        ]" />
        <!-- Main Content -->
    ...
    <div id="addSubmissionModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30 hidden">
        <div class="bg-white rounded-lg shadow-lg p-8 w-full max-w-md">
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-lg font-semibold">Add Submission</h2>
                <button onclick="hideAddSubmissionModal()" class="text-gray-400 hover:text-gray-700">&times;</button>
            </div>
            <form id="addSubmissionForm" onsubmit="submitSubmission(event)">
                <div class="mb-4">
                    <label class="block text-sm font-medium mb-1">Submission Note</label>
                    <input type="text" id="submissionNote" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none" required>
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-medium mb-1">Upload Files</label>
                    <input type="file" id="submissionFiles" class="w-full" multiple required>
                </div>
                <div class="flex justify-end">
                    <button type="submit" class="px-4 py-2 bg-[#7bbde8] text-white rounded hover:bg-[#5ca6d6]">Submit</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
// Dummy data for tasks assigned to 'You'
let myTasks = [
    { name: 'Task A', status: 'Ongoing', date: '2024-06-10', submission: null },
    { name: 'Task B', status: 'Not Started', date: '2024-06-12', submission: null },
    { name: 'Task C', status: 'Done', date: '2024-06-08', submission: { note: 'Sudah submit', files: ['file1.pdf', 'file2.png'] } },
];
let currentTaskIndex = null;

function renderMyTasks() {
    const tbody = document.getElementById('my-task-table-body');
    tbody.innerHTML = myTasks.map((t, idx) => `
        <tr class="border-b border-gray-200">
            ...
            <td class="py-3 px-4">
                ${t.submission
                    ? `<div><div class='text-xs text-gray-700 mb-1'>${t.submission.note}</div>${t.submission.files.map(f => `<span class='inline-block bg-[#eaf6fd] text-[#7bbde8] rounded px-2 py-1 text-xs mr-1 mb-1'>${f}</span>`).join('')}</div>`
                    : `<button onclick="showAddSubmissionModal(${idx})" class="px-3 py-1 bg-[#7bbde8] text-white rounded text-xs">Add Submission</button>`
                }
            </td>
        </tr>
    `).join('');
}
function showAddSubmissionModal(idx) {
